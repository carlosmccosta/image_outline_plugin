...
// =============================================================================  <public-section>  ============================================================================
// >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>   <functions>   <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
ImageOutlinePlugin::ImageOutlinePlugin() :
		image_count_(0),
...
void ImageOutlinePlugin::OnNewFrame(const unsigned char *_image, unsigned int _width, unsigned int _height, unsigned int _depth, const std::string &_format) {
	...
	if (format == "R8G8B8") {
		cv::Mat source_image(_height, width, CV_8UC3, (void*)_image);
		cv::Mat image;
		cv::cvtColor(source_image, image, CV_RGB2HSV);
		SegmentImageHSV(image);
		ProcessContours(image);
	}
}


void ImageOutlinePlugin::SegmentImageHSV(cv::Mat& _image_hsv) {
...
void ImageOutlinePlugin::ProcessContours(cv::Mat& _binary_image) {
